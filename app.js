...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. LANDING & WORKSPACE TRANSITION CONTROLLERS
  const landingPage = document.getElementById('landingPage');
  const appWorkspace = document.getElementById('appWorkspace');
  const btnStartApp = document.getElementById('btnStartApp');
  const btnBackHome = document.getElementById('btnBackHome');
  const btnOpenSkill = document.getElementById('btnOpenSkill');
  const skillModal = document.getElementById('skillModal');
  const btnCloseSkill = document.getElementById('btnCloseSkill');

  // Start App clicked
  btnStartApp.addEventListener('click', () => {
    landingPage.classList.add('hidden');
    appWorkspace.classList.remove('hidden');
    showToast('已進入比對校對工作區！');
  });
